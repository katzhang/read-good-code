...
## It's ok to do conditional importing in Python (import inside try-except blocks)
## http://stackoverflow.com/questions/3131217/error-handling-when-importing-modules
try:
...
## Pygments: a package that can highlight programming language syntax
...
## Where are os env variables are set?
...
## Similar to JS, None, 0, '' are evaluated to be False
...
## Programming style: define constants in the beginning
...
def is_question(link):
    return re.search('questions/\d+/', link) ## Stackoverflow's question url


def get_links(query):
    result = get_result(SEARCH_URL.format(URL, url_quote(query))) ## Get the response data given a query
    html = pq(result) ## pyquery: sort of like Python's jQuery
...
def command_line_runner():
    parser = get_parser()
    ## vars: Return the __dict__ for a module, class, instance or any other object with that attribute
